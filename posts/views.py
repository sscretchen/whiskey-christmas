...
def blog(request):
    '''Logic for seeing all members posts'''
...
class IndexView(ListView):
    """ Main page view with top posts, categories, tags and more. """
    template_name = "posts/index.html"
    model = Post
    context_object_name = 'posts'
    paginate_by = 3
    # Sort by most recent F-I-L-O
    ordering = ['-timestamp']
    ...
    def form_valid(self, form):
        ...
        for tag in tags:
            ...
            if current_tag.count() < 1:
                # the create function 'title' arg is from the Tag model field 'title'
                create_tag = Tag.objects.create(title=tag)
                form.instance.tag.add(create_tag)
            else:
                database_tag = Tag.objects.get(slug=slugify(tag))
                form.instance.tag.add(database_tag)
